...
pub type SpecialFormHandler = fn(&mut Evaluator, &[EvalObject]) -> Command;
/// Native handler for an interface form.
pub type InterfaceFormHandler = fn(&[EvalObject]) -> Command;
...
impl Binding {
    ...
    ///
    pub fn as_constant(&self) -> Option<&EvalObject> {
        match self {
    ...
    /// Gets the constant's value from the binding.
    ///
    pub fn as_constant_mut(&mut self) -> Option<&mut EvalObject> {
        match self {
            Self::Constant(v) => Some(v),
            _ => None,
        }
    ...
    ///
    pub fn as_variable(&self) -> Option<&EvalObject> {
        match self {
    ...
    /// Gets the variable's value from the binding.
    ///
    pub fn as_variable_mut(&mut self) -> Option<&mut EvalObject> {
        match self {
            Self::Variable(v) => Some(v),
            _ => None,
        }
    ...
    /// Gets the constant's value from the binding.
    ///
    pub fn get_constant(&self) -> &EvalObject {
        self.as_constant().expect("expected a constant binding")
    }

    /// Gets the constant's value from the binding.
    ///
    pub fn get_constant_mut(&mut self) -> &mut EvalObject {
        self.as_constant_mut().expect("expected a constant binding")
    }

    /// Gets the variable's value from the binding.
    ///
    pub fn get_variable(&self) -> &EvalObject {
        self.as_variable().expect("expected a variable binding")
    }

    /// Gets the variable's value from the binding.
    ///
    pub fn get_variable_mut(&mut self) -> &mut EvalObject {
...
impl<T> NativeForm<T> {
    ...
    ///
    pub fn new(handler: T, argument_count: i32, argument_kinds: Vec<Kind>) -> Self {
        Self {
            handler,
            argument_count,
            argument_kinds,
...
impl LispForm {
    ...
    ///
    pub fn expressions(&self) -> &Vec<EvalObject> {
        &self.expressions
    }
}
